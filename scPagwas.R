#我参考：https://www.writebug.com/article/18857056-434a-11ee-9ee5-0242ac1c0011#snp%E5%88%B0%E9%80%9A%E8%B7%AF%E7%9A%84%E6%98%A0%E5%B0%84
...
#prepare sc data
str <- "~/dat/Global atlas/All_rds/annotation_rds/"
setwd("~/dat/Global atlas/All_rds/annotation_rds")
list0 <- list.files(pattern = ".rds")
list0 <- sapply(list0, function(x) unlist(strsplit(x, "\\."))[1])   #将文件名按.分割，取分割后的第一个元素，即去掉扩展名的文件名。
list0 <- data.frame(list0)
tissue <- list0$list0  #提取文件名列
list1<-NULL      
for(i in tissue){
  list1[[i]]<-paste0(str, i, ".rds")  #生成新的带路径的文件名
}
...
#chrom_ld = chrom_ld,# The LD data is provided by package.直接用package提供就行
#这里去解释的是人类的GWAS，所以就用人类的就好



#prepare pathway data
#Genes_by_pathway_kegg <- list(  #这是我猜长这样
...
for (i in 1:length(Genes_by_pathway_kegg)) {
  list1 <- data.frame(Genes_by_pathway_kegg[i])  #Genename 
  list1$group <- names(Genes_by_pathway_kegg[i]) #KEGG
  names(list1)[1] <- "gene"
...
pathway <- pathway[,-1]
kegg <- split(pathway$gene, pathway$group) #按group组分割基因
#我猜
#$Pathway1
#[1] "GeneA" "GeneB" "GeneC"
...
for (k in 1:length(tissue)) {
  ...
  Pagwas <- Pathway_pcascore_run(
   ...
   #输入GWAS数据.注意，事实上，这里的maf不会对结果造成很大影响。
  ...
  #SNP到基因的映射,marg可以设置windowsize，实际上在一些文献中会设计成10k；
  ...
  #SNP到通路的映射。这个步骤能够把通路注释到基因组上的区域。
  Pagwas <- Pathway_annotation_input(
    Pagwas = Pagwas,
    block_annotation = block_annotation
  )
 
 #将通路的基因组位置和前面得到的pca矩阵（实质上是通路组）进行联系      
